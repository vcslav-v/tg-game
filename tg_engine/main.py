...
async def get_user_context(
    ...
    if force_link:
        next_message = await db_tools.get_message(force_link)
    ...
    if not user_context:
        ...
        context.user_data['user_context'] = schemas.UserContext(
            next_message=await db_tools.get_message(cur_message)
        )
        user_context: schemas.UserContext = context.user_data.get('user_context')
    return user_context
...
async def echo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for button in user_context.next_message.buttons:
        if update.message.text != button.text:
            continue
        user_context.next_message = await db_tools.get_message(button.next_message_link)
        context.user_data['user_context'] = user_context
        break
...
async def send_message(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await db_tools.set_cur_message(
            context.job.chat_id,
            message.link,
            True if message.start_of_chapter_name else False,
        )

    if message.next_msg:
        message = await db_tools.get_message(message.next_msg)
...
async def add_to_queue(
    ...
    if not next_message:
        ...
        next_message = await db_tools.get_message(cur_message_link)
...
async def set_story(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with io.BytesIO() as zip_file:
        await story_file.download(out=zip_file)
...
async def jump(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    started_chapters = await db_tools.get_started_chapters(update.effective_message.chat_id)
    keyboard = []
    for chapter_name, message_link in started_chapters:
        keyboard.append([
            InlineKeyboardButton(chapter_name, callback_data=f'jump_to={message_link}')]
        )
    keyboard.append([
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if command == 'jump_to':
        await add_to_queue(update.effective_message.chat_id, context, value)
